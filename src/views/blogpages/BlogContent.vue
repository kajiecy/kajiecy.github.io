  ...
  <div class="blog-content">
    <div class="blog-content-body">
      <div class="img-div">
        ...
        <template v-else>
          <div
            style="width: 100%; height: 100%; background-color: #e4e3e6"
          ></div>
        </template>
        <div class="title-div">
          {{ blogContent.title }}
      ...
      <div class="tag-div">
        <span
          v-if="blogContent.created_at"
          class="tag-item"
          style="background-color: #258efb"
          ><i class="iconfont icon-riqi"></i
          >{{ formatDate(blogContent.created_at) }}</span
        >
        <span
          ...
          ><i class="iconfont icon-riqi"></i
          >{{ getCharCount(blogContent.body) }}字</span
        >
        <span
          v-if="blogContent.body"
          class="tag-item"
          ...
          {{ calcReadTime(blogContent.body) }} 分钟</span
        >
        <span
          v-if="blogContent.labels && blogContent.labels.length"
          class="tag-item blog-labels"
          v-for="cItem in blogContent.labels.filter(
        ...
        <span
          v-if="blogContent.milestone"
          class="tag-item"
          style="background-color: #67c23a; cursor: pointer"
          @click="
            $router.push({
              name: 'blog_list',
              query: { milestone: blogContent.milestone.number },
            })
          "
          ><i class="iconfont icon-leimupinleifenleileibie"></i
          >{{ blogContent.milestone.title }}</span
        >
      </div>
      <article
        v-if="markDownBody"
      ...
      <div class="do-comment">
        <div class="title">
          <span class="right-span">
            ...
            <span
              v-if="!loginInfo.accessToken"
              class="login-button"
              @click="toLogin"
            >
              登录
            </span>
            <span v-else class="login-button" @click="loginOut"> 退出 </span>
          </span>
        </div>
        <div class="body dis_table wd100">
          <div class="dis_table_cell">
            <i
              v-if="!loginInfo.accessToken"
              class="iconfont icon-wode-weidenglu"
            ></i>
          ...
          <div class="dis_table_cell">
            <div class="preview-switch">
              <span
                ...
                @click="changePreviewMode(false)"
                >编辑</span
              >
              <span class="halving-line">|</span>
              <span
                :class="!previewMode ? '' : 'active'"
                @click="changePreviewMode(true)"
                >预览</span
              >
            </div>
              ...
              <textarea
                ...
                :placeholder="
                  !loginInfo.accessToken
                    ...
                    : '欢迎留言评论！'
                "
                :disabled="!loginInfo.accessToken"
              ></textarea>
              <div class="comment_button">
                <span @click="sendComment"> 评 论 </span>
              </div>
            </template>
            <template v-else>
              <div
            ...
            <div class="tips">注：评论功能现已全面支持 Markdown 语法！</div>
          </div>
        </div>
      </div>
      <div class="blog-comment">
        <div class="title">
          <span> 评论 ({{ blogContent.comments }}) </span>
        </div>
        <div class="body">
          <template v-if="commentList.length">
            <div
              class="item dis_table wd100"
              v-for="(item, index) in commentList"
              :key="index"
            >
              <div
                class="dis_table_cell comment_user_avatar"
                style="width: 44px; vertical-align: top"
              >
                <img
                  width="44"
                  height="44"
              ...
              <div class="table-right dis_table_cell textleft vertical-top">
                ...
                <div
                  class="comment_content markdown-body"
                  v-html="item.body"
                ></div>
              </div>
            </div>
          ...
          <page-component
            v-if="blogContent.comments"
            :pageNum="pageNum"
            :pageSize="pageSize"
            :total="blogContent.comments"
...
export default class BlogList extends Vue {
    ...
    if (
      ...
      (async () => {
        ...
        localStorage.setItem(
          GithubConfig.LOCALSTORAGE_LOGIN_AVATAR,
          loginUserInfo.avatar_url
        );
  ...
  get loginInfo() {
    return {
      accessToken: localStorage.getItem(GithubConfig.LOCALSTORAGE_NAME),
      loginAvatar: localStorage.getItem(GithubConfig.LOCALSTORAGE_LOGIN_AVATAR),
    };
  }
  async initData(
    { toLastPage }: { toLastPage: boolean } = { toLastPage: false }
  ) {
    (async () => {
      if (
        this.$store.state.templateIssuesInfo.id &&
        this.$store.state.templateIssuesInfo.id ===
          this.$route.query.issueNumber
      ) {
        this.blogContent = this.$store.state.templateIssuesInfo;
      } else {
        this.blogContent = await this.$githubApi.getIssuesContent({
          issueNumber: this.$route.query.issueNumber,
        });
      }
      this.markDownBody = <string>(
        await this.$githubApi.getMdContent({ text: this.blogContent.body })
      );
      this.markDownBody = this.markDownBody.replace(
        /<a href="([^"]*).* src="([^"]*).* alt="([^"]*)".* data-canonical-src="([^"]*)".*<\/a>/gi,
        `<a href="$4" target="_blank" rel="nofollow"><img src="$4" alt="$3" data-canonical-src="$4" style="max-width:100%;"></a>`
      );
      if (toLastPage) {
  ...
  async getCommentList() {
    this.commentList = await this.$githubApi.getCommentList4Issues({
      issueNumber: this.$route.query.issueNumber,
      page: this.pageNum,
  ...
  async sendComment() {
    let markDownCommentBody = <string>(
      await this.$githubApi.getMdContent({ text: this.commentBody })
    );
    await this.$githubApi.createComment({
      comment: markDownCommentBody,
      issueNumber: this.blogContent.number,
    });
    this.commentBody = "";
